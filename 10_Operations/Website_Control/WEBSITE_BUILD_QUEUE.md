...
| HIGH | CEH Standard | Public surface audit cleanup | Review page ownership, canonicals, metadata, and route assumptions | IN PROGRESS | Codex | `CEHSTANDARD_PAGE_INVENTORY.md` now includes live route checks plus source-file ownership for most pages. Next step: confirm `benchmark-index` source path, then decide handling for `/ceh-index`, `/capital-stack`, and `/pages/partners`. |
...
| MEDIUM | CEH Standard | Standards page inventory | List all current pages and missing pages | IN PROGRESS | Codex | First-pass route and source-file inventory mostly complete. Expand after `benchmark-index` source confirmation and mixed-surface classification decisions. |
